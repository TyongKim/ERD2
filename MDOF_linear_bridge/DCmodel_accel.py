...
dataloader2 = DataLoader(dataset, batch_size=512*10, shuffle=False)

#%% Load DNN model
model = DNNModel()
model.load_state_dict(torch.load('./DL_data/DNN_accel_wotime_gm.pt'))
model.eval()
...
error_DNN_MSE_mm =  np.mean((DNN_results_MDOF - Results_SDOF)**2)
error_SRSS_MSE_mm = np.mean((DNN_results_MDOF - Results_SRSS)**2)
...
print('Dataset error: MSE')
print(error_DNN_MSE_test)
print(error_SRSS_MSE_test)

print('Dataset error: MSE; m/s^2 units')
print(error_DNN_MSE_mm)
print(error_SRSS_MSE_mm)

print('Dataset error: RE')
print(error_DNN)
print(error_SRSS)

print('Dataset error: RE max')
