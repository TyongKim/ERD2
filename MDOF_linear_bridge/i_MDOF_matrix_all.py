...
def modal_analysis_ver3(stiff, mass, damping_coef):
    ...
    # Gamma value
    G = []
    for kk in range(len(stiff)):
        tmp_G = np.sum(L[kk,:])/M[kk,kk]
        G.append(tmp_G)
    ...
    M_eff = []
    for kk in range(len(stiff)):
        M_eff.append(np.sum(L[kk,:]*G[kk]))

    M_eff_norm = M_eff/np.sum(mass)    
    ...
    tmp2_Str = np.r_[natural_period, M_eff_norm, damping]    
